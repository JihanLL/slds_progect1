...
def train_loop(
    dataloader, base_model, model_ema, loss_fn, optimizer, scheduler, epoch, l1_lambda
):
    ...
    for loop in range(1):
        ...
        if flopanalysis:
            ...
            flops_analysis = FlopCountAnalysis(
                base_model, torch.randn(8, 1, 28, 28).to(device)
            )
        ...
        model_ema = AveragedModel(
            base_model,
            multi_avg_fn=torch.optim.swa_utils.get_ema_multi_avg_fn(L2_parameter),
        ...
        for t in range(epochs):
            ...
            test_loop(
                test_dataloader, model_ema, loss_fn, t == epochs - 1
